...
.DESCRIPTION
  Sets up GlazeWM + Zebar + the bindings from this repo. Touches nothing else
  on Windows — no wallpaper change, no accent colour, no dark-theme flip, no
  taskbar tweak, no Windows Terminal theming. Pure tiler + topbar + bindings.
...
# ─── 3. Fonts (optional, only what zebar/glazewm need to look right) ──────
if (-not $NoFonts) {
    Write-Step 'Installing RobotoMono Nerd Font (for the topbar)'
    ...
    Write-Ok 'RobotoMono Nerd Font ready.'
}
